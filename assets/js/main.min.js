...
let pigeonSpawnCount = 50; /* TEMP: testing starting value, revert to 0 */
const pigeonCounterEl = document.createElement('div');
pigeonCounterEl.id = 'pigeon-spawn-counter';
pigeonCounterEl.style.cssText =
  'position:absolute; top:0; left:0; width:100%; height:100%; z-index:5; ' +
  'display:flex; align-items:center; justify-content:center; ' + /* TEMP: forced visible for debugging, revert to display:none */
  'pointer-events:none; color:#fff; font-weight:bold; font-size:12px; ' +
  'text-shadow:0 0 3px #000, 0 0 3px #000;';
pigeonCounterEl.textContent = pigeonSpawnCount; /* TEMP: initial text so it's visible at 0 */

const pigeonAvatarEl = document.querySelector('.author__avatar');
if (pigeonAvatarEl) {
  pigeonAvatarEl.style.position = 'relative';
  pigeonAvatarEl.appendChild(pigeonCounterEl);
}

function bumpPigeonCounter() {
  pigeonSpawnCount++;
  pigeonCounterEl.textContent = pigeonSpawnCount; /* TEMP: always update, revert to gated update inside threshold check */
  if (pigeonSpawnCount >= 10) { /* TEMP: testing threshold, revert to 100 */
    pigeonCounterEl.style.display = 'flex';
  }
}
...
style2.textContent = `
  .pop-image2 {
    position: fixed;
    pointer-events: none;
    animation: popSquiggle 1s ease forwards;
  }

  @keyframes popSquiggle {
    0%   { 
      opacity: 0; 
    ...
    30%  { 
      opacity: 0.3: 
      transform: translate(calc(-50% + var(--wobble-x)), calc(-50% + var(--wobble-y))) rotate(calc(var(--rotation) + 20deg)); 
    }
    50%  { 
      transform: translate(calc(-50% + var(--drift-x)), calc(-50% + var(--drift-y))) rotate(calc(var(--rotation) - 15deg)); 
    }
    100% { 
      opacity: 0; 
      transform: translate(calc(-50% + var(--exit-x)), calc(-50% + var(--exit-y))) scale(0.5) rotate(calc(var(--rotation) + 30deg)); 
    }
  }
`;
document.head.appendChild(style2);

const vibrate = document.createElement('style');
vibrate.textContent = `
.vibrate {
  animation: vibrate 0.3s linear infinite,
  buildUp 3s ease forwards;
}
...
@keyframes buildUp {
  ...
  100% { filter: brightness(3);    }  /* flash white at the end */
}
`;
document.head.appendChild(vibrate);


function spawnImage(x, y) {
  bumpPigeonCounter();
  const img = document.createElement('img');
//   img.src = '/images/popin/pidge1.png';
  img.src = '/images/popin/pidge' + parseInt(Math.random()*14) +'.png';
  img.classList.add('pop-image2');
  img.style.left = x + 'px';
  img.style.top  = y + 'px';
  // img.style.width = '100px';
  img.style.width = parseInt(Math.random()*50 + 50) + 'px';
  ...
  const exitDist = 150 + Math.random() * 100;

  // Wobble points along the way (perpendicular drift for squiggle effect)
  const perpAngle = angle + Math.PI / 2;
  const wobbleAmount = 30 + Math.random() * 50;

  img.style.setProperty('--rotation',  `${rotation}deg`);
  img.style.setProperty('--wobble-x',  `${Math.cos(perpAngle) * wobbleAmount}px`);
  img.style.setProperty('--wobble-y',  `${Math.sin(perpAngle) * wobbleAmount}px`);
  img.style.setProperty('--drift-x',   `${Math.cos(angle) * exitDist * 0.5 - Math.cos(perpAngle) * wobbleAmount}px`);
  img.style.setProperty('--drift-y',   `${Math.sin(angle) * exitDist * 0.5 - Math.sin(perpAngle) * wobbleAmount}px`);
  img.style.setProperty('--exit-x',    `${Math.cos(angle) * exitDist}px`);
  img.style.setProperty('--exit-y',    `${Math.sin(angle) * exitDist}px`);
...
addEventListener("scroll", (event) => { 
  const now = Date.now();
  if (now - lastScroll < 50) return; // only fire every 100ms
  lastScroll = now;
  if(mouseX != -1000){
  spawnImage(mouseX, mouseY);
    popfunc(mouseX, mouseY);
  }
})



// const peekMirror = document.createElement('style');
// peekMirror.textContent = `
// .peeker {
//   position: fixed;
//   bottom: 0;
//   right: 0;
...
//   animation: peek 3s ease infinite;
// }
// @keyframes peek {
//   0%   { transform: translate(30%, 100%) scaleX(-1) }  /* hidden */
//   20%  { transform: translate(30%, 40%)  scaleX(-1) }  /* peek up */
//   40%  { transform: translate(30%, 40%)  scaleX(-1) }  /* hold */
//   60%  { transform: translate(30%, 100%) scaleX(-1) }  /* retreat */
//   100% { transform: translate(30%, 100%) scaleX(-1) }  /* wait before next cycle */
// }
// `;
// document.head.appendChild(peekMirror);
// const peeker = document.createElement('img');
// peeker.src = '/images/peek.png';
// peeker.classList.add('peeker');
// document.body.appendChild(peeker);



function createPeeker(side) {
  ...
  style.textContent = `
    .peeker-${side} {
      ...
      ${side}: 0;
      width: 150px;
      animation: peek-${side} 30s ease infinite;
    }

    @keyframes peek-${side} {
      0%   { transform: translate(${side === 'right' ? '30%' : '-30%'}, 100%) scaleX(${side === 'right' ? '-1' : '1'}); }
      80%   { transform: translate(${side === 'right' ? '30%' : '-30%'}, 100%) scaleX(${side === 'right' ? '-1' : '1'}); }
      92%  { transform: translate(${side === 'right' ? '30%' : '-30%'}, 40%)  scaleX(${side === 'right' ? '-1' : '1'}); }
      95%  { transform: translate(${side === 'right' ? '30%' : '-30%'}, 40%)  scaleX(${side === 'right' ? '-1' : '1'}); }
      96%  { transform: translate(${side === 'right' ? '30%' : '-30%'}, 100%) scaleX(${side === 'right' ? '-1' : '1'}); }
      100% { transform: translate(${side === 'right' ? '30%' : '-30%'}, 100%) scaleX(${side === 'right' ? '-1' : '1'}); }
    }
  `;
  document.head.appendChild(style);

  const img = document.createElement('img');
  img.src = '/images/peek.png';
  img.classList.add(`peeker-${side}`);
...
if(Math.random() > 0.9){
  createPeeker("left");
  console.log("TWO PIDGES");
  setTimeout(() => {
    createPeeker('right');
  }, 15000);
} else {
  createPeeker(Math.random() > 0.5 ? 'right' : 'left');
}
}, 1000);


function updateAddress(id, appleLink, androidLink){
  let elem = document.getElementById(id);
  if(elem != null){
    console.log("found " + id + " to update address");
    ...
    if(useAppleMaps){
      ...
      elem.href = androidLink;
    } else {
      elem.href = appleLink;
    }
  } else {
    console.log("DID NOT FIND " + id + " to update address");
  }
}
document.addEventListener('DOMContentLoaded', () => {
  updateAddress('tbc-address', 'https://maps.apple.com/?address=1010%20Santa%20Clara%20Pl,%20San%20Diego,%20CA%20%2092109,%20United%20States&auid=6661340332734406382&ll=32.785146,-117.249109&lsp=9902&q=Tower%20Beach%20Club&t=m','https://maps.app.goo.gl/hJYF9yfQVzyM3YgHA')
  updateAddress('stone-address', 'https://maps.apple.com/?address=2816%20Historic%20Decatur%20Rd,%20San%20Diego,%20CA%2092106,%20United%20States&auid=3160733903697935298&ll=32.740396,-117.211563&lsp=9902&q=Stone%20Brewing&t=m', 'https://maps.app.goo.gl/AhmkDb64aCznckpG6')
});